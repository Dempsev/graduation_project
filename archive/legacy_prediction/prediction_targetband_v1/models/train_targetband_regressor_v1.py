...
DEFAULT_DATASET = ROOT / 'data' / 'prediction_targetband_v1' / 'band120_160' / 'targetband_prediction_v1.csv'
DEFAULT_OUT_ROOT = ROOT / 'data' / 'prediction_targetband_v1_runs'
TARGET_CHOICES = ['target_gap_overlap_Hz', 'target_gap_cover_ratio']


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train a pure-structural regressor for fixed target-band overlap strength.')
    parser.add_argument('--dataset', type=Path, default=DEFAULT_DATASET)
    parser.add_argument('--target', default='target_gap_cover_ratio', choices=TARGET_CHOICES)
    ...
    parser.add_argument('--min-positive-stage-rows', type=int, default=5)
    parser.add_argument('--run-name', default='targetband_cover_v1')
...
def build_strat_labels(y: np.ndarray) -> np.ndarray:
    ...
    n_bins = int(min(4, np.unique(y).size))
    try:
...
def main() -> None:
    ...
    if not feature_cols:
        raise RuntimeError('No usable feature columns found for target-band regressor.')
    if len(df) == 0:
        raise RuntimeError('No positive rows available for target-band regression.')
    ...
    save_csv_rows(
        ...
        ['fold', 'sample_id', 'design_id', 'source_stage', 'shape_id', 'shape_family', 'point_id', 'target_band_low_Hz', 'target_band_high_Hz', 'target_name', 'y_true', 'y_pred', 'abs_error'],
        prediction_rows,
    )
    save_json(
    ...
    print('[DONE] target-band regression training complete')
