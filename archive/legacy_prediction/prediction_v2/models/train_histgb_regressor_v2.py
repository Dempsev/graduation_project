...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train a HistGradientBoosting pure-prediction regressor on the aggregated v2 dataset.')
    ...
    parser.add_argument('--run-name', default='histgb_gap34width_v2')
    parser.add_argument('--target-transform', default='auto', choices=['auto', 'none', 'log1p'])
...
def resolve_transform(mode: str, y: np.ndarray) -> str:
    if mode != 'auto':
        return mode
    return 'log1p' if float(np.nanmin(y)) >= 0.0 else 'none'
...
def make_model(args: argparse.Namespace) -> HistGradientBoostingRegressor:
...
def run_fold(
    ...
    pred_test = inverse_target(model.predict(x_test), target_transform)
    pred_test = np.clip(pred_test, 0.0, None) if target_transform == 'log1p' else pred_test
...
def main() -> None:
    ...
    if not feature_cols:
        raise RuntimeError('No usable feature columns found for v2 HistGB regressor.')

    target_transform = resolve_transform(args.target_transform, df[args.target].astype(float).to_numpy())
    ...
    for fold_name, train_idx, test_idx in split_iter:
        ...
        metrics, rows = run_fold(df, train_idx, test_idx, feature_cols, args, fold_name, target_transform, fold_dir)
        fold_metrics.append({
            'fold': fold_name,
            'rows': len(test_idx),
            **metrics,
    ...
    print('[DONE] prediction_v2 HistGB training complete')
