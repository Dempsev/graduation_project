...
DEFAULT_DATASET = ROOT / 'data' / 'pure_prediction_v6' / 'v1' / 'pure_firstgap_regression_v6.csv'
DEFAULT_OUT_ROOT = ROOT / 'data' / 'pure_prediction_v6_runs'
TARGET_CHOICES = ['first_gap_width_Hz', 'first_gap_width_rel', 'first_gap_center_freq']


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train a pure-structural regressor on the first-complete-gap v6 dataset.')
    parser.add_argument('--dataset', type=Path, default=DEFAULT_DATASET)
    parser.add_argument('--target', default='first_gap_width_Hz', choices=TARGET_CHOICES)
    ...
    parser.add_argument('--run-name', default='rf_firstgap_width_v6')
...
def run_fold(
    ...
    for idx, row in test_df.iterrows():
        rows.append({
            ...
            'first_gap_lower_band': float(row.get('first_gap_lower_band', np.nan)),
            'first_gap_upper_band': float(row.get('first_gap_upper_band', np.nan)),
            'first_gap_center_freq': float(row.get('first_gap_center_freq', np.nan)),
...
def main() -> None:
    ...
    if not feature_cols:
        raise RuntimeError('No usable feature columns found for v6 first-gap regressor.')
    ...
    save_csv_rows(
        ...
        ['fold', 'sample_id', 'design_id', 'source_stage', 'shape_id', 'shape_family', 'point_id', 'first_gap_lower_band', 'first_gap_upper_band', 'first_gap_center_freq', 'target_name', 'y_true', 'y_pred', 'abs_error'],
        prediction_rows,
    )
    save_json(
    ...
    print('[DONE] prediction_v6 first-gap RandomForest training complete')
