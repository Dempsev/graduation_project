...
DEFAULT_DATASET = ROOT / 'data' / 'pure_prediction_v7' / 'v1' / 'cap250' / 'pure_maxgap_regression_v7.csv'
DEFAULT_OUT_ROOT = ROOT / 'data' / 'pure_prediction_v7_runs'
TARGET_CHOICES = ['max_gap_below_cap_width_Hz', 'max_gap_below_cap_width_rel', 'max_gap_below_cap_center_freq']


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train a pure-structural regressor on the max-gap-below-cap v7 dataset.')
    parser.add_argument('--dataset', type=Path, default=DEFAULT_DATASET)
    parser.add_argument('--target', default='max_gap_below_cap_width_Hz', choices=TARGET_CHOICES)
    ...
    parser.add_argument('--run-name', default='rf_maxgap_width_v7')
...
def build_strat_labels(y: np.ndarray) -> np.ndarray:
    ...
    try:
        bins = pd.qcut(y, q=min(5, np.unique(y).size), labels=False, duplicates='drop')
...
def run_fold(
    ...
    for idx, row in test_df.iterrows():
        rows.append({
            ...
            'max_gap_below_cap_lower_band': float(row.get('max_gap_below_cap_lower_band', np.nan)),
            'max_gap_below_cap_upper_band': float(row.get('max_gap_below_cap_upper_band', np.nan)),
            'max_gap_below_cap_center_freq': float(row.get('max_gap_below_cap_center_freq', np.nan)),
...
def main() -> None:
    ...
    if not feature_cols:
        raise RuntimeError('No usable feature columns found for v7 max-gap regressor.')
    ...
    save_csv_rows(
        ...
        ['fold', 'sample_id', 'design_id', 'source_stage', 'shape_id', 'shape_family', 'point_id', 'max_gap_below_cap_lower_band', 'max_gap_below_cap_upper_band', 'max_gap_below_cap_center_freq', 'target_name', 'y_true', 'y_pred', 'abs_error'],
        prediction_rows,
    )
    save_json(
    ...
    print('[DONE] prediction_v7 max-gap RandomForest training complete')
