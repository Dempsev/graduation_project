...
policyPath = fullfile(rootDir, 'stage3_training', 'policies', 'seed_discovery_v10.json');
...
if ~isfile(policyPath)
    error('run_stage3_run_seed_discovery_scoring_v10:MissingPolicy', 'Policy JSON not found: %s', policyPath);
end
cmd = sprintf('python "%s" --dataset "%s" --run-name candidate_pool_seed_discovery_v10 --policy-json "%s"', scriptPath, datasetPath, policyPath);
status = system(cmd);
if status ~= 0
