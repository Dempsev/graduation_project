...
```powershell
python scripts\build_dataset\build_parametric_targetband_dataset_v1.py `
...
```powershell
python scripts\train_prediction\train_parametric_targetband_classifier_v1.py `
...
```powershell
python scripts\train_prediction\train_parametric_targetband_regressor_v1.py `
  --dataset data/prediction_targetband_param_v1/v1/windows_dense_v8_truth_plus_exploratory_aug_v1/targetband_parametric_v1.csv `
...
```powershell
python scripts\run_ga\score_targetband_candidates_v1.py `
...
```powershell
python scripts\run_ga\run_targetband_local_ga_v1.py `
...
```powershell
python scripts\run_ga\build_targetband_validation_manifest_v1.py `
...
```matlab
run(fullfile(pwd, 'scripts', 'run_comsol', 'run_comsol_stage4_targetband_top6_v1.m'));
```
...
contractPath = fullfile(pwd, 'src', 'shared', 'contracts', ...
...
```powershell
python scripts\export_results\build_thesis_application_bundle_v1.py
```
