function cfg = get_comsol_in_loop_ga_band_supplement_config_v1()
%GET_COMSOL_IN_LOOP_GA_BAND_SUPPLEMENT_CONFIG_V1
% Focused real COMSOL-in-loop GA for supplementing weak thesis target bands.

cfg = get_comsol_in_loop_ga_global_config_v1();
cfg.gaId = 'comsol_in_loop_band_supplement_ga_v1';
cfg = apply_real_ga_output_layout_v1(cfg, fullfile(cfg.rootDir, 'data', 'comsol_batch', cfg.gaId));

% Prioritize weak thesis bands discovered by coverage audit:
% 1) 200-240, 2) 240-280, 3) 220-260, 4) 180-220.
cfg.bandCatalog = build_priority_band_catalog();
cfg.bandSelectionMode = 'rotate_by_generation';
cfg.archiveTopCandidatesPerBand = 8;
cfg.topCandidatesExport = 32;

% Give supplementation runs a slightly longer budget so weak bands have
% enough generations to surface useful archives.
cfg.maxGenerations = 28;
cfg.generations = cfg.maxGenerations;
cfg.earlyStopMinDeltaFitness = 0.0008;
cfg.earlyStopPatience = 10;
cfg.earlyStopMinGenerations = 16;
...
signatureParts = [ ...
    ...
    'band_catalog_role=targeted_supplementation', ...
...
function catalog = build_priority_band_catalog()
catalog = repmat(struct('bandTag', '', 'bandLowHz', 0, 'bandHighHz', 0), 4, 1);
catalog(1) = make_band('band200_240', 200, 240);
catalog(2) = make_band('band240_280', 240, 280);
catalog(3) = make_band('band220_260', 220, 260);
catalog(4) = make_band('band180_220', 180, 220);
end
