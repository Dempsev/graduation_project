...
DEFAULT_BASE_CSV = (
    ROOT / "data" / "analysis" / "targetband_family_expansion_probe_v1" / "targetband_family_expansion_probe_merged_v1.csv"
)
DEFAULT_OUT_DIR = (
    ROOT / "data" / "ml_runs" / "targetband_family_expansion_probe_round2_v1" / "validation_manifest_v1"
)
...
ROUND2_RECIPES_BY_GROUP: Dict[str, List[Tuple[str, Dict[str, float], str]]] = {
    "r0_family": [
        ("round2_center", {}, "round2_center"),
        ("r0_plus_tinier", {"r0": 0.00006}, "radius_followup"),
        ("r0_minus_tinier", {"r0": -0.00006}, "radius_followup"),
        ("a1_plus_tiny", {"a1": 0.0020}, "direction_followup"),
        ("a2_plus_tiny", {"a2": 0.0020}, "direction_followup"),
        ("r0_plus_tinier_a1_plus_tiny", {"r0": 0.00006, "a1": 0.0020}, "coupled_followup"),
        ("r0_plus_tinier_a2_plus_tiny", {"r0": 0.00006, "a2": 0.0020}, "coupled_followup"),
    ],
    "a1a2_family": [
        ("round2_center", {}, "round2_center"),
        ("a1_plus_tiny", {"a1": 0.0020}, "direction_followup"),
        ("a1_minus_tiny", {"a1": -0.0020}, "direction_followup"),
        ("a2_minus_tiny", {"a2": -0.0020}, "direction_followup"),
        ("a2_plus_tiny", {"a2": 0.0020}, "direction_followup"),
        ("b2_plus_tiny", {"b2": 0.0020}, "direction_followup"),
        ("a1_plus_tiny_a2_minus_tiny", {"a1": 0.0020, "a2": -0.0020}, "coupled_followup"),
    ],
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build round-2 family-expansion probe manifest.")
...
def group_for_target(best_variant: str) -> str:
    if best_variant == "a1_plus_a2_minus":
        return "a1a2_family"
    return "r0_family"
...
def build_row(seed: Dict[str, object], variant_label: str, deltas: Dict[str, float], preferred_direction: str, rank: int) -> Dict[str, object]:
    ...
    out["validation_id"] = f"{band_tag}__{family}__round2__{rank:02d}"
    out["selection_source"] = "targetband_family_expansion_probe_round2_v1"
    out["selection_label"] = f"{band_tag}__{family}__family_probe_round2"
    out["rank_within_source"] = rank
    out["pool_arm"] = "targetband_family_expansion_probe_round2"
    out["point_strategy"] = "family_directed_probe_round2"
    out["target_rule"] = "weak_band_family_expansion_round2_v1"
    out["step_window"] = "family_expansion_probe_round2"
    ...
    out["round2_parent_validation_id"] = str(seed.get("validation_id", ""))
    out["round2_parent_variant"] = str(seed.get("family_probe_variant", ""))
...
def main() -> None:
    ...
    for best in best_rows:
        ...
        recipe_group = group_for_target(best_variant)
        seed_summary.append(
            {
                "target_band_tag": str(best["target_band_tag"]),
                "shape_family": str(best["shape_family"]),
                "shape_id": str(best["shape_id"]),
                "base_validation_id": str(best["validation_id"]),
                "base_best_variant": best_variant,
        ...
        for variant_label, deltas, preferred_direction in ROUND2_RECIPES_BY_GROUP[recipe_group]:
            rows.append(build_row(best, variant_label, deltas, preferred_direction, rank))
            rank += 1

    manifest = pd.DataFrame(rows)
    manifest_path = out_dir / "targetband_family_expansion_probe_round2_manifest_v1.csv"
    manifest.to_csv(manifest_path, index=False, encoding="utf-8-sig")

    pd.DataFrame(seed_summary).to_csv(
        out_dir / "targetband_family_expansion_probe_round2_seeds_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    summary = {
        ...
        "variants_per_family_target": int(len(ROUND2_RECIPES_BY_GROUP["r0_family"])),
        ...
        "recipe_groups": {
            name: [
                ...
                for label, deltas, direction in recipes
            ]
            for name, recipes in ROUND2_RECIPES_BY_GROUP.items()
        },
        "seeds": seed_summary,
    }
    (out_dir / "targetband_family_expansion_probe_round2_manifest_summary_v1.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    print("[DONE] target-band family expansion probe round2 manifest built")
