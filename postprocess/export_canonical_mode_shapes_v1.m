function export_canonical_mode_shapes_v1()
%EXPORT_CANONICAL_MODE_SHAPES_V1 Rebuild center canonical cases and export band-edge mode shapes.
%
% This helper is intentionally scoped to the four center canonical cases from
% the local robustness study. It:
% 1. reads the merged robustness table to find center-case parameters;
% 2. rebuilds the COMSOL model for each center case;
% 3. reruns the eigenfrequency sweep and regenerates tbl1;
% 4. locates the lower/upper edge selectors of the fixed 3-4 band gap;
% 5. exports 2D mode-shape images for the lower and upper edges.
%
% Expected environment:
% - MATLAB with COMSOL LiveLink available
% - repository root on disk as in the current project checkout
...
mergedCsv = fullfile(rootDir, 'data', 'analysis', 'canonical_local_robustness_v1', 'canonical_local_robustness_merged_v1.csv');
outDir = fullfile(rootDir, 'data', 'analysis', 'canonical_mode_shapes_v1');
ensure_dir(outDir);

if ~isfile(mergedCsv)
    error('export_canonical_mode_shapes_v1:MissingMergedCsv', 'Merged robustness csv not found: %s', mergedCsv);
end
...
if isempty(rows)
    error('export_canonical_mode_shapes_v1:NoCenterRows', 'No center rows found in merged robustness csv.');
end
...
for i = 1:height(rows)
    row = rows(i, :);
    caseId = char(string(row.canonical_case_id(1)));
    sampleId = char(string(row.sample_id(1)));
    shapeId = char(string(row.shape_id(1)));
    fprintf('[%d/%d] exporting mode shapes for %s (%s)\n', i, height(rows), caseId, shapeId);

    pointSpec = build_point_spec_from_row(row);
    shapeFile = fullfile(cfg.shapeDir, [shapeId '.csv']);
    if ~isfile(shapeFile)
        warning('export_canonical_mode_shapes_v1:MissingShapeFile', 'Shape file missing for %s: %s', caseId, shapeFile);
        continue;
    end
    ...
    if ~report.geometry_valid || ~report.contact_valid || isempty(model)
        warning('export_canonical_mode_shapes_v1:GeometryInvalid', ...
            'Geometry/contact invalid for %s: %s', caseId, char(string(report.error_message)));
        continue;
    end
    ...
    try
        ...
        try
            ...
            warning('export_canonical_mode_shapes_v1:BatchFallback', ...
                'batch compute failed for %s, fallback to study.run: %s', caseId, MEComp.message);
        ...
        if ~isfile(tbl1Path)
            error('export_canonical_mode_shapes_v1:MissingTbl1', 'tbl1 export missing: %s', tbl1Path);
        end

        selectors = locate_fixed_gap_edge_selectors_from_tbl1(tbl1Path, cfg.fixedGapBand);
        ...
        export_mode_shape_image(model, selectors.lower, lowerOut, sprintf('%s lower edge', caseId));
        export_mode_shape_image(model, selectors.upper, upperOut, sprintf('%s upper edge', caseId));

        summaryRows = append_summary(summaryRows, struct( ...
            'canonical_case_id', string(caseId), ...
            ...
            'lower_edge_band_index', selectors.lower.band_index, ...
            ...
            'upper_edge_band_index', selectors.upper.band_index, ...
        ...
        warning('export_canonical_mode_shapes_v1:CaseFailed', 'Case %s failed: %s', caseId, ME.message);
    end
...
if ~isempty(summaryRows)
    ...
    writetable(summaryTable, fullfile(outDir, 'canonical_mode_shape_export_summary_v1.csv'));
end
end
...
function selectors = locate_fixed_gap_edge_selectors_from_tbl1(tbl1Path, fixedGapBand)
...
if maxBands < fixedGapBand + 1
    error('locate_fixed_gap_edge_selectors_from_tbl1:TooFewBands', ...
        'Need at least %d bands in %s', fixedGapBand + 1, tbl1Path);
end
...
lowerBand = bandMatrix(:, fixedGapBand);
upperBand = bandMatrix(:, fixedGapBand + 1);
...
try
    % Different LiveLink versions accept different mphplot name-value pairs.
    % To stay compatible, configure the plot group first, then call mphplot
