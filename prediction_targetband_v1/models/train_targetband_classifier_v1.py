...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train a pure-structural classifier for fixed target-band gap existence.')
    ...
    parser.add_argument('--run-name', default='targetband_cls_v1')
...
def make_model(args: argparse.Namespace) -> RandomForestClassifier:
...
def run_fold(
    ...
    for idx, row in test_df.iterrows():
        rows.append({
            ...
            'y_pred': int(pred[idx]),
            'target_gap_overlap_Hz': float(row['target_gap_overlap_Hz']),
        })
    return metrics, rows
...
def main() -> None:
    ...
    if not feature_cols:
        raise RuntimeError('No usable feature columns found for target-band classifier.')
    ...
    save_csv_rows(
        ...
        ['fold', 'sample_id', 'design_id', 'source_stage', 'shape_id', 'shape_family', 'point_id', 'target_band_low_Hz', 'target_band_high_Hz', 'y_true', 'y_prob', 'y_pred', 'target_gap_overlap_Hz'],
        prediction_rows,
    )
    save_json(
    ...
    print('[DONE] target-band classifier training complete')
