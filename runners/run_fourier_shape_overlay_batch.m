...
outDir = fullfile(rootDir, 'data', 'comsol_batch', 'fourier_shape_overlay');
modelsDir = fullfile(outDir, 'models');
logDir = fullfile(outDir, 'logs');
tbl1Dir = fullfile(outDir, 'tbl1_exports');
errorLogCsv = fullfile(logDir, 'run_fourier_shape_overlay_errors.csv');
manifestCsv = fullfile(outDir, 'case_manifest.csv');

startIndex = 1;
maxCount = 4; % baseline + a few initial snake overlays by default
doCompute = true;
saveModel = true;
runPostprocess = true;
pythonCmd = 'python';
...
for i = startIndex:endIndex
    ...
    if useDiscretePerturbation
        shapeFile = resolve_case_shape_file(caseRow, shapeDir, rootDir);
    end
    ...
    assign_overlay_context(shapeFile, caseId, paramOverrides, useDiscretePerturbation, studyNeigs, studyShift);

    try
        modelTag = ['overlay' num2str(i)];
        model = ModelUtil.create(modelTag);
        model.modelPath(outDir);
        model.label(['fourier_overlay_' caseId]);
...
disp("Shape overlay batch finished.");
...
required = {'case_id', 'use_discrete_shape', 'neigs', 'shift_Hz', 'a1'};
...
for name = {'neigs', 'shift_Hz', 'a1', 'b1', 'a2', 'b2', 'a3', 'b3'}
    col = name{1};
    if ~ismember(col, cases.Properties.VariableNames)
        continue;
    end
...
function shapeFile = resolve_case_shape_file(caseRow, shapeDir, rootDir)
shapeFile = '';
...
if isempty(shapeFile)
    error("shape_file is required when use_discrete_shape=1");
end
...
function assign_overlay_context(shapeFile, caseId, paramOverrides, useDiscretePerturbation, studyNeigs, studyShift)
