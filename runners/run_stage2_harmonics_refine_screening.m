...
maxCandidateCount = 0; % set positive value to limit candidate subset
startPointIndex = 1;
maxPointCount = 0; % set positive value to limit DOE subset

candidateTable = generate_stage2_harmonics_refine_candidate_manifest(cfg);
doeTable = get_stage2_harmonics_refine_doe_points(cfg);

if isempty(candidateTable)
    error('run_stage2_harmonics_refine_screening:EmptyCandidates', 'No harmonic-refine candidates were generated.');
end
if isempty(doeTable)
    error('run_stage2_harmonics_refine_screening:EmptyDoe', 'Harmonic-refine DOE is empty.');
end
...
fprintf('Stage-2 harmonic refine screening\n');
fprintf('Fourier mother: %s\n', cfg.fourierId);
fprintf('Material case: %s\n', cfg.materialCase);
fprintf('Source harmonics summary: %s\n', cfg.stage2HarmonicsShapeSummaryCsv);
...
baselineByPoint = evaluate_stage2_harmonics_refine_baseline_points(cfg, doeTable);
runResults = struct([]);
resumeSkipped = 0;
totalTasks = 0;
for i = 1:height(activeCandidates)
    totalTasks = totalTasks + nnz(ismember(string(activePoints.main_id), split_allowed_main_ids(activeCandidates.allowed_main_ids(i))));
end
taskCounter = 0;
...
for i = 1:height(activeCandidates)
    ...
    allowedMainIds = split_allowed_main_ids(candidateStruct.allowed_main_ids);
    candidatePoints = activePoints(ismember(string(activePoints.main_id), allowedMainIds), :);
    fprintf('\nRefine candidate [%d/%d] %s (%s, mains=%s)\n', ...
        i, height(activeCandidates), char(string(candidateStruct.shape_id)), ...
        char(string(candidateStruct.shape_role)), char(string(candidateStruct.allowed_main_ids)));

    for j = 1:height(candidatePoints)
        taskCounter = taskCounter + 1;
        pointSpec = point_row_to_struct(candidatePoints(j, :));
        ...
        fprintf(['  [%d/%d] point=%s main=%s a1=%.3f a2=%.3f b2=%.3f r0=%.4f ' ...
                 'b3=%.3f a4=%.3f b5=%.3f\n'], ...
            taskCounter, totalTasks, pointSpec.point_id, pointSpec.main_id, ...
            pointSpec.a1, pointSpec.a2, pointSpec.b2, pointSpec.r0, pointSpec.b3, pointSpec.a4, pointSpec.b5);

        result = evaluate_stage2_harmonics_refine_single_sample(cfg, candidateStruct, pointSpec, refPoint);
        ...
        write_stage2_harmonics_refine_summary_tables(resultsTable, cfg);

        fprintf('    geometry_valid=%s, contact_valid=%s, solve_success=%s, gap34_gain_Hz=%s, max_gap=%s (%s-%s)\n', ...
            ...
            numeric_text(result.max_gap_Hz), numeric_text(result.max_gap_lower_band), numeric_text(result.max_gap_upper_band));
...
write_stage2_harmonics_refine_summary_tables(allResultsTable, cfg);

fprintf('\nStage-2 harmonic refine run summary\n');
...
candidate = struct( ...
    ...
    'allowed_main_ids', string(row.allowed_main_ids(1)) ...
);
end
...
if isempty(idx)
    error('run_stage2_harmonics_refine_screening:MissingBaselinePoint', 'No baseline reference found for point %s', pointId);
end
...
function mainIds = split_allowed_main_ids(text)
parts = regexp(char(string(text)), '\s*,\s*', 'split');
parts = parts(~cellfun(@isempty, parts));
mainIds = string(parts(:));
end
