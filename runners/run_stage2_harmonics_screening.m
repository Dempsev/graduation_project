...
cfg = get_stage2_harmonics_config();
...
candidateTable = generate_stage2_harmonics_candidate_manifest(cfg);
doeTable = get_stage2_harmonics_doe_points(cfg);

if isempty(candidateTable)
    error('run_stage2_harmonics_screening:EmptyCandidates', ...
        'No stage2_harmonics candidates were generated from %s', cfg.refineShapeSummaryCsv);
end
if isempty(doeTable)
    error('run_stage2_harmonics_screening:EmptyDoe', 'Stage2_harmonics DOE is empty.');
end
...
baselineByPoint = evaluate_stage2_harmonics_baseline_points(cfg, doeTable);
runResults = struct([]);
resumeSkipped = 0;
...
for i = 1:height(activeCandidates)
    ...
    fprintf('\nHarmonic candidate [%d/%d] %s (%s, family=%s)\n', ...
    ...
    for j = 1:height(activePoints)
        ...
        fprintf(['  [%d/%d] point=%s main=%s a1=%.3f a2=%.3f b2=%.3f r0=%.4f ' ...
                 'a3=%.3f b3=%.3f a4=%.3f b4=%.3f a5=%.3f b5=%.3f\n'], ...
            taskCounter, totalTasks, pointSpec.point_id, pointSpec.main_id, ...
            pointSpec.a1, pointSpec.a2, pointSpec.b2, pointSpec.r0, ...
            pointSpec.a3, pointSpec.b3, pointSpec.a4, pointSpec.b4, pointSpec.a5, pointSpec.b5);

        result = evaluate_stage2_harmonics_single_sample(cfg, candidateStruct, pointSpec, refPoint);
        ...
        write_stage2_harmonics_summary_tables(resultsTable, cfg);
...
write_stage2_harmonics_summary_tables(allResultsTable, cfg);

fprintf('\nStage-2 harmonics run summary\n');
...
if isempty(idx)
    error('run_stage2_harmonics_screening:MissingBaselinePoint', ...
...
for i = 1:numel(cfg.resultFieldOrder)
    ...
    switch fieldName
        case {'sample_id', 'candidate_id', 'shape_id', 'shape_family', 'candidate_role', 'main_id', 'point_id', 'material_case', 'error_message'}
            out.(fieldName) = "";
