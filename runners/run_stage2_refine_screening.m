...
addpath(genpath(fullfile(rootDir, 'stage2_refine')));

cfg = get_stage2_refine_config();
...
candidateTable = generate_stage2_refine_candidate_manifest(cfg);
doeTable = get_stage2_refine_doe_points(cfg);

if isempty(candidateTable)
    error('run_stage2_refine_screening:EmptyCandidates', ...
        'No stage2_refine candidates were generated from %s', cfg.stage2ShapeSummaryCsv);
end
if isempty(doeTable)
    error('run_stage2_refine_screening:EmptyDoe', 'Stage2_refine DOE is empty.');
end
...
for i = 1:height(activeCandidates)
    ...
    fprintf('\nRefine candidate [%d/%d] %s (%s, family=%s)\n', ...
...
if isempty(idx)
    error('run_stage2_refine_screening:MissingBaselinePoint', ...
