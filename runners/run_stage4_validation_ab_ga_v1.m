...
cfg = get_stage4_validation_config_ga_v1();
startIndex = 1;
maxCount = 0; % set positive value to limit validation subset

if ~isfile(cfg.validationManifestCsv)
    error('run_stage4_validation_ab_ga_v1:MissingManifest', 'Validation manifest not found: %s', cfg.validationManifestCsv);
end
...
fprintf('Stage-4 validation A/B run (GA)\n');
...
if isempty(idx)
    error('run_stage4_validation_ab_ga_v1:MissingBaselinePoint', 'No baseline reference found for point %s', pointId);
end
...
result.rank_cascade = numeric_or_nan_from_field(row, 'rank_cascade');
result.rank_surrogate = numeric_or_nan_from_field(row, 'rank_surrogate');
result.source_sample_id = string_or_empty(row, 'sample_id');
