...
cfg = get_stage4_validation_config();
startIndex = 1;
maxCount = 0; % set positive value to limit validation subset

if ~isfile(cfg.validationManifestCsv)
    error('run_stage4_validation_ab_v1:MissingManifest', 'Validation manifest not found: %s', cfg.validationManifestCsv);
end
...
if isempty(idx)
    error('run_stage4_validation_ab_v1:MissingBaselinePoint', 'No baseline reference found for point %s', pointId);
end
