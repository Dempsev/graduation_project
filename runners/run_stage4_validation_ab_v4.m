...
cfg = get_stage4_validation_config_v4();
startIndex = 1;
maxCount = 0; % set positive value to limit validation subset

if ~isfile(cfg.validationManifestCsv)
    error('run_stage4_validation_ab_v4:MissingManifest', 'Validation manifest not found: %s', cfg.validationManifestCsv);
end
...
textVars = {'validation_id','selection_source','selection_label','sample_id','shape_id','shape_family','shape_role','candidate_id','main_id','point_id','pool_arm','point_strategy','family_prior_source'};
...
if isempty(idx)
    error('run_stage4_validation_ab_v4:MissingBaselinePoint', 'No baseline reference found for point %s', pointId);
end
...
if ismember('rank_cascade', row.Properties.VariableNames)
    result.rank_cascade = numeric_or_nan(row.rank_cascade(1));
else
    result.rank_cascade = NaN;
end
if ismember('rank_surrogate', row.Properties.VariableNames)
    result.rank_surrogate = numeric_or_nan(row.rank_surrogate(1));
else
    result.rank_surrogate = NaN;
end
end
...
stringFields = { ...
    ...
    'shape_id','shape_family','shape_role','candidate_id','main_id','point_id','pool_arm','point_strategy','family_prior_source','material_case','error_message' ...
};
