...
cfg = get_stage4_validation_config_v5();
startIndex = 1;
maxCount = 0; % set positive value to limit validation subset

if ~isfile(cfg.validationManifestCsv)
    error('run_stage4_validation_ab_v5:MissingManifest', 'Validation manifest not found: %s', cfg.validationManifestCsv);
end
...
textVars = {'validation_id','selection_source','selection_label','sample_id','shape_id','shape_family','shape_role','candidate_id','main_id','point_id','pool_arm','point_strategy','family_prior_source','seed_prior_source','seed_shape_id','seed_family','seed_tier','seed_source','step_window'};
...
if isempty(idx)
    error('run_stage4_validation_ab_v5:MissingBaselinePoint', 'No baseline reference found for point %s', pointId);
end
...
stringFields = { ...
    ...
    'seed_shape_id','seed_family','seed_tier','seed_source','step_window', ...
