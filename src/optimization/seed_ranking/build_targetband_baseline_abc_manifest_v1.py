...
DEFAULT_SCORED_CSV = (
    ...
    / 'data'
    / 'ml_runs'
    / 'targetband_seed_scoring_v1'
    / 'band180_220'
    / 'targetband_seed_predictions.csv'
)
DEFAULT_LOCAL_GA_MANIFEST = (
    ROOT
    / 'data'
    / 'ml_runs'
    / 'targetband_local_ga_v1'
    / 'band180_220_center_top6'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Build a shared Stage4 manifest for random, predictor-only, and predictor+local-GA target-band baselines.'
    )
    ...
    parser.add_argument('--local-ga-manifest', type=Path, default=DEFAULT_LOCAL_GA_MANIFEST)
...
def filter_point(df: pd.DataFrame, point_id: str) -> pd.DataFrame:
    ...
    if out.empty:
        raise RuntimeError(f'No rows found for point_id={point_id}')
    return out
...
def family_balanced_random(df: pd.DataFrame, k: int, seed: int) -> pd.DataFrame:
    ...
    ).map(lambda value: stable_key(value, rng_seed))
    ...
    if len(work) < k:
        ...
        ).map(lambda value: stable_key(value, rng_seed + 1))
...
def main() -> None:
    ...
    local_ga_manifest_path = resolve_path(args.local_ga_manifest)
    for path in [candidate_pool_path, scored_path, local_ga_manifest_path]:
        if not path.exists():
    ...
    local_ga = pd.read_csv(local_ga_manifest_path)

    random_rows = family_balanced_random(candidate_pool, k, int(args.random_seed))
    predictor_rows = predictor_only_topk(scored, k)
    local_ga_rows = local_ga.head(k).copy()

    arms = [
        arm_frame(random_rows, 'random_family_balanced_v1', f'random_family_balanced_k{k}_seed_{args.random_seed}', 'rand', k),
        arm_frame(predictor_rows, 'predictor_only_topk_v1', f'predictor_only_180_220_top_{k}', 'pred', k),
        arm_frame(local_ga_rows, 'predictor_local_ga_v1', f'predictor_local_ga_180_220_top_{k}', 'hybrid', k),
    ]
    manifest = pd.concat(arms, ignore_index=True, sort=False)

    manifest_path = out_dir / 'targetband_baseline_abc_manifest_v1.csv'
    ...
    summary = {
        ...
        'local_ga_manifest': str(local_ga_manifest_path),
        'manifest_csv': str(manifest_path),
        'rows_total': int(len(prepared)),
        'arms': [
            summarize_arm(random_rows, 'random_family_balanced_v1'),
            summarize_arm(predictor_rows, 'predictor_only_topk_v1'),
            summarize_arm(local_ga_rows, 'predictor_local_ga_v1'),
        ],
    }
    summary_path = out_dir / 'targetband_baseline_abc_manifest_summary.json'
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding='utf-8')

    print('[DONE] target-band ABC validation manifest built')
