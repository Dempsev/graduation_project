...
SOURCE_DIR = ROOT / "data" / "ml_runs" / "targetband_seed_scoring_v11_12gen_freeze_multiband_neighborhood_v1"
OUT_DIR = ROOT / "data" / "ml_runs" / "targetband_multiband_predictor_top1_v11_12gen_freeze_v1" / "validation_manifest_v1"
...
def build_row(band_tag: str, band_label: str, rank: int, source: pd.Series) -> dict:
    row = source.to_dict()
    row["validation_id"] = f"mbv11_{rank:03d}"
    row["selection_source"] = "predictor_top1_multiband_v11_12gen_freeze_v1"
    ...
    row["candidate_id"] = f"mbv11_{band_tag}_top1"
...
def main() -> None:
    ...
    manifest_path = OUT_DIR / "targetband_multiband_predictor_top1_v11_12gen_freeze_manifest_v1.csv"
    manifest.to_csv(manifest_path, index=False, encoding="utf-8-sig")

    summary = {
        "source_dir": str(SOURCE_DIR),
        "manifest_rows": int(len(manifest)),
        "selection": "top-1 candidate per thesis target band, sorted by v11 12-generation-freeze predicted target overlap",
        "bands": [{"band_tag": tag, "band_label": label} for tag, label in BANDS],
        "manifest_csv": str(manifest_path),
    }
    (OUT_DIR / "targetband_multiband_predictor_top1_v11_12gen_freeze_manifest_summary_v1.json").write_text(
