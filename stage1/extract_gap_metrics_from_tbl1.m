function metrics = extract_gap_metrics_from_tbl1(tbl1Path)
%EXTRACT_GAP_METRICS_FROM_TBL1 Compute stage-1 gap labels from COMSOL tbl1.
% The gap definition intentionally follows the current project postprocess
% logic instead of introducing branch tracking:
% 1) For each k-point, frequencies are sorted in ascending order to form
%    band1, band2, ...
...
if ~isfile(tbl1Path)
    error('extract_gap_metrics_from_tbl1:MissingFile', 'tbl1 csv not found: %s', tbl1Path);
end

[kVals, freqVals] = read_tbl1_numeric(tbl1Path);
if isempty(kVals)
    error('extract_gap_metrics_from_tbl1:EmptyData', 'No numeric rows found in: %s', tbl1Path);
end
...
if fid < 0
    error('extract_gap_metrics_from_tbl1:OpenFailed', 'Failed to open: %s', tbl1Path);
end
