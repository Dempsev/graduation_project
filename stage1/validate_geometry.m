function [report, model] = validate_geometry(cfg, shapeFile, sampleId)
%VALIDATE_GEOMETRY Build the stage-1 geometry and return screening checks.
% Geometry validity is intentionally coarse in v1:
% - contact_valid follows the current project rule used by build_geom_02.m
% - has_tiny_fragments is inferred from abnormal domain counts only
% - contact_length is an approximate arc-length estimate along the Fourier
%   boundary, based on sampled point-to-polyline distance
...
shapeCheck = inspect_shape_contact(cfg, shapeFile);
...
try
    ...
    assign_stage1_context(cfg, sampleId, shapeFile);
...
function out = inspect_shape_contact(cfg, shapeFile)
out = struct( ...
    'file_valid', true, ...
    'contact_valid', isempty(shapeFile), ...
    'contact_length', NaN, ...
...
fourierXY = sample_fourier_boundary(cfg);
...
function xy = sample_fourier_boundary(cfg)
t = linspace(0, 2 * pi, cfg.contactSampleCount + 1)';
r0 = cfg.paramNumeric.r0;
a = zeros(1, 5);
b = zeros(1, 5);
a(1) = cfg.paramNumeric.a1;
a(2) = cfg.paramNumeric.a2;
a(3) = cfg.paramNumeric.a3;
a(4) = cfg.paramNumeric.a4;
a(5) = cfg.paramNumeric.a5;
b(1) = cfg.paramNumeric.b1;
b(2) = cfg.paramNumeric.b2;
b(3) = cfg.paramNumeric.b3;
b(4) = cfg.paramNumeric.b4;
b(5) = cfg.paramNumeric.b5;
