function result = evaluate_stage2_gapdiversity_case_internal(cfg, sampleMeta, pointSpec, refPoint)
%EVALUATE_STAGE2_GAPDIVERSITY_CASE_INTERNAL Shared worker for exploration baseline and shape cases.
...
[geometryReport, model] = validate_stage2_gapdiversity_geometry(cfg, pointSpec, shapeFile, result.sample_id);
...
try
    ...
    try
        ...
        warning('evaluate_stage2_gapdiversity_case_internal:BatchFallback', ...
            'batch compute failed for %s, fallback to study.run: %s', result.sample_id, MEComp.message);
    ...
    if ~isfile(tbl1Path)
        error('evaluate_stage2_gapdiversity_case_internal:MissingTbl1', 'Expected tbl1 export not found: %s', tbl1Path);
    end
    ...
    result.gap_lower_edge_Hz = gapMetrics.gap_lower_edge_Hz;
    result.gap_upper_edge_Hz = gapMetrics.gap_upper_edge_Hz;
...
for name = {'r0','a1','b1','a2','b2','a3','b3','a4','b4','a5','b5','A1','phi1','A2','phi2','A3','phi3','A4','phi4','A5','phi5','sumA','deriv_budget','min_radius_est','max_radius_est','max_abs_amp_slope'}
    key = char(name{1});
    if isfield(pointSpec, key)
        result.(key) = pointSpec.(key);
    end
end
result.guardrail_pass = logical(pointSpec.guardrail_pass);
