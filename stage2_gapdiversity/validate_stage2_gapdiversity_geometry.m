function [report, model] = validate_stage2_gapdiversity_geometry(cfg, pointSpec, shapeFile, sampleId)
%VALIDATE_STAGE2_GAPDIVERSITY_GEOMETRY Build one exploration geometry and return screening checks.
...
if nargin < 4 || isempty(sampleId)
    sampleId = 'stage2_gapdiversity_sample';
end
...
try
    ...
    assign_stage2_gapdiversity_context(cfg, pointSpec, sampleId, shapeFile);
...
amp = ones(size(t));
for k = 1:5
    amp = amp + pointSpec.(sprintf('a%d', k)) .* cos(k * t) + pointSpec.(sprintf('b%d', k)) .* sin(k * t);
end
r = pointSpec.r0 .* amp;
xy = [r .* cos(t), r .* sin(t)];
end
...
try
    if evalin('base', sprintf('exist(''%s'',''var'')', varName))
        ...
        if isstring(value) || ischar(value), s = char(string(value)); end
    end
catch
end
end
