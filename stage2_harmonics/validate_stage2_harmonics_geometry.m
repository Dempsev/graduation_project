function [report, model] = validate_stage2_harmonics_geometry(cfg, pointSpec, shapeFile, sampleId)
%VALIDATE_STAGE2_HARMONICS_GEOMETRY Build one harmonic-screen geometry and check contact.
...
if nargin < 4 || isempty(sampleId)
    sampleId = 'stage2_harmonics_sample';
end
...
try
    ...
    assign_stage2_harmonics_context(cfg, pointSpec, sampleId, shapeFile);
...
report.has_tiny_fragments = isfinite(report.n_domains) && report.n_domains > cfg.tinyFragmentDomainThreshold;
...
out = struct('file_valid', true, 'contact_valid', isempty(shapeFile), 'contact_length', NaN, 'min_distance', NaN, 'error_message', '');
if isempty(shapeFile)
    return;
end
if ~isfile(shapeFile)
...
params.a3 = pointSpec.a3;
params.b3 = pointSpec.b3;
params.a4 = pointSpec.a4;
params.b4 = pointSpec.b4;
params.a5 = pointSpec.a5;
params.b5 = pointSpec.b5;

a = [params.a1, params.a2, params.a3, params.a4, params.a5];
b = [params.b1, params.b2, params.b3, params.b4, params.b5];
amp = ones(size(t));
for k = 1:5
...
for i = 1:size(poly1,1)-1
    p1 = poly1(i,:); q1 = poly1(i+1,:);
    for j = 1:size(poly2,1)-1
        p2 = poly2(j,:); q2 = poly2(j+1,:);
...
eps0 = 1e-12;
o1 = orient2d(p1,q1,p2); o2 = orient2d(p1,q1,q2);
o3 = orient2d(p2,q2,p1); o4 = orient2d(p2,q2,q1);
if (o1*o2 < 0) && (o3*o4 < 0)
    tf = true; return;
end
if abs(o1)<eps0 && on_segment(p1,q1,p2,eps0), tf = true; return; end
if abs(o2)<eps0 && on_segment(p1,q1,q2,eps0), tf = true; return; end
if abs(o3)<eps0 && on_segment(p2,q2,p1,eps0), tf = true; return; end
if abs(o4)<eps0 && on_segment(p2,q2,q1,eps0), tf = true; return; end
tf = false;
end
...
try
    ...
    entities = [];
    if isfield(intSel,'entities'), entities = [entities; double(intSel.entities(:))]; end
    if isfield(difSel,'entities'), entities = [entities; double(difSel.entities(:))]; end
    if ~isempty(entities), n = numel(unique(entities)); end
catch
end
end
