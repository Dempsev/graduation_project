...
ROOT = base.ROOT
OUT_DIR = ROOT / 'data' / 'ml_dataset' / 'v2'
...
SURROGATE_CORE_STAGES = ['stage2', 'stage2_refine', 'stage2_harmonics', 'stage2_harmonics_refine', 'stage4_validation']
PARAM_CLASSIFICATION_STAGES = SURROGATE_CORE_STAGES
SPECIALCASE_SHAPE_FAMILIES = {'ep209'}

MASTER_CSV = OUT_DIR / 'master_dataset_v2.csv'
REGRESSION_CSV = OUT_DIR / 'mlp_gap34_regression_v2.csv'
STAGE_SUMMARY_CSV = OUT_DIR / 'dataset_stage_summary_v2.csv'
DATASET_INFO_JSON = OUT_DIR / 'dataset_info_v2.json'

CONTACT_TASK_CSV = TASKS_DIR / 'shape_screening_contact_cls_v2.csv'
POSITIVE_TASK_CSV = TASKS_DIR / 'shape_screening_positive_cls_v2.csv'
PARAM_CONTACT_TASK_CSV = TASKS_DIR / 'parametric_contact_cls_v2.csv'
PARAM_POSITIVE_TASK_CSV = TASKS_DIR / 'parametric_positive_cls_v2.csv'
SURROGATE_CORE_TASK_CSV = TASKS_DIR / 'surrogate_regression_core_v2.csv'
SURROGATE_SPECIALCASE_TASK_CSV = TASKS_DIR / 'surrogate_regression_specialcase_v2.csv'
...
def to_optional_float(value):
    value = base.to_float(value)
    return value


def standardize_row_v2(stage_cfg: Dict[str, object], row: Dict[str, str]) -> Dict[str, object]:
...
def build_rows() -> List[Dict[str, object]]:
    ...
    for stage_cfg in STAGES:
        for row in base.read_csv_rows(Path(stage_cfg['results_csv'])):
            rows.append(standardize_row_v2(stage_cfg, row))
...
def build_task_datasets(rows: List[Dict[str, object]]) -> Dict[str, List[Dict[str, object]]]:
    ...
    param_contact_rows = [
        row for row in rows
        if row['source_stage'] in PARAM_CLASSIFICATION_STAGES
    ]
    param_positive_rows = [
        row for row in rows
        if row['source_stage'] in PARAM_CLASSIFICATION_STAGES and int(row['contact_valid']) == 1 and int(row['solve_success']) == 1
    ]
    surrogate_core_rows = [
        row for row in rows
...
def build_dataset_info(rows: List[Dict[str, object]], regression_rows: List[Dict[str, object]], stage_summary: List[Dict[str, object]], task_rows: Dict[str, List[Dict[str, object]]]) -> Dict[str, object]:
    return {
        ...
        'tasks': {
            'shape_screening_contact_cls_v2': {
            ...
            'shape_screening_positive_cls_v2': {
            ...
            'parametric_contact_cls_v2': {
                'path': str(PARAM_CONTACT_TASK_CSV),
                'rows': len(task_rows['param_contact_cls']),
                'target': 'contact_valid',
                'source_stages': PARAM_CLASSIFICATION_STAGES,
                'feature_preset': 'parametric_core',
            },
            'parametric_positive_cls_v2': {
                'path': str(PARAM_POSITIVE_TASK_CSV),
                'rows': len(task_rows['param_positive_cls']),
                'target': 'is_positive_shape',
                'source_stages': PARAM_CLASSIFICATION_STAGES,
                'feature_preset': 'parametric_core',
                'row_filter': 'contact_valid=1 && solve_success=1',
