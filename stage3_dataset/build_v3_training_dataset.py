...
ROOT = base.ROOT
OUT_DIR = ROOT / 'data' / 'ml_dataset' / 'v3'
...
SURROGATE_CORE_STAGES = ['stage2', 'stage2_refine', 'stage2_harmonics', 'stage2_harmonics_refine', 'stage4_validation_v1', 'stage4_validation_v2']
PARAM_CLASSIFICATION_STAGES = SURROGATE_CORE_STAGES
SPECIALCASE_SHAPE_FAMILIES = {'ep209'}

MASTER_CSV = OUT_DIR / 'master_dataset_v3.csv'
REGRESSION_CSV = OUT_DIR / 'mlp_gap34_regression_v3.csv'
STAGE_SUMMARY_CSV = OUT_DIR / 'dataset_stage_summary_v3.csv'
DATASET_INFO_JSON = OUT_DIR / 'dataset_info_v3.json'

CONTACT_TASK_CSV = TASKS_DIR / 'shape_screening_contact_cls_v3.csv'
POSITIVE_TASK_CSV = TASKS_DIR / 'shape_screening_positive_cls_v3.csv'
PARAM_CONTACT_TASK_CSV = TASKS_DIR / 'parametric_contact_cls_v3.csv'
PARAM_POSITIVE_TASK_CSV = TASKS_DIR / 'parametric_positive_cls_v3.csv'
SURROGATE_CORE_TASK_CSV = TASKS_DIR / 'surrogate_regression_core_v3.csv'
SURROGATE_SPECIALCASE_TASK_CSV = TASKS_DIR / 'surrogate_regression_specialcase_v3.csv'
...
def standardize_row_v3(stage_cfg: Dict[str, object], row: Dict[str, str]) -> Dict[str, object]:
...
def build_rows() -> List[Dict[str, object]]:
    ...
    for stage_cfg in STAGES:
        for row in base.read_csv_rows(Path(stage_cfg['results_csv'])):
            rows.append(standardize_row_v3(stage_cfg, row))
...
def build_dataset_info(rows: List[Dict[str, object]], regression_rows: List[Dict[str, object]], stage_summary: List[Dict[str, object]], task_rows: Dict[str, List[Dict[str, object]]]) -> Dict[str, object]:
    return {
        ...
        'tasks': {
            'shape_screening_contact_cls_v3': {'path': str(CONTACT_TASK_CSV), 'rows': len(task_rows['contact_cls']), 'target': 'contact_valid', 'source_stages': ['stage1'], 'feature_preset': 'shape_only'},
            'shape_screening_positive_cls_v3': {'path': str(POSITIVE_TASK_CSV), 'rows': len(task_rows['positive_cls']), 'target': 'is_positive_shape', 'source_stages': ['stage1'], 'feature_preset': 'shape_only', 'row_filter': 'contact_valid=1 && solve_success=1'},
            'parametric_contact_cls_v3': {'path': str(PARAM_CONTACT_TASK_CSV), 'rows': len(task_rows['param_contact_cls']), 'target': 'contact_valid', 'source_stages': PARAM_CLASSIFICATION_STAGES, 'feature_preset': 'parametric_core'},
            'parametric_positive_cls_v3': {'path': str(PARAM_POSITIVE_TASK_CSV), 'rows': len(task_rows['param_positive_cls']), 'target': 'is_positive_shape', 'source_stages': PARAM_CLASSIFICATION_STAGES, 'feature_preset': 'parametric_core', 'row_filter': 'contact_valid=1 && solve_success=1'},
            'surrogate_regression_core_v3': {'path': str(SURROGATE_CORE_TASK_CSV), 'rows': len(task_rows['surrogate_core']), 'target': 'gap34_gain_Hz', 'source_stages': SURROGATE_CORE_STAGES, 'feature_presets': ['surrogate_core', 'surrogate_geo_augmented'], 'row_filter': 'is_training_ready=1 && not special_case'},
            'surrogate_regression_specialcase_v3': {'path': str(SURROGATE_SPECIALCASE_TASK_CSV), 'rows': len(task_rows['surrogate_specialcase']), 'target': 'gap34_gain_Hz', 'source_stages': SURROGATE_CORE_STAGES, 'feature_presets': ['surrogate_core', 'surrogate_geo_augmented'], 'row_filter': 'shape_family in special_case_set && is_training_ready=1'},
        },
