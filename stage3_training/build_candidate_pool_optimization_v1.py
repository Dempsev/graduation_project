...
if __name__ == '__main__':
    result = build_candidate_pool_for_profile(get_profile('candidate_pool_optimization_v1'))
    print(f"[DONE] excluded families: {len(result['excluded_families'])}")
    print(f"[DONE] seed candidates: {len(result['seed_manifest'])}")
    print(f"[DONE] point specs: {len(get_profile('candidate_pool_optimization_v1')['point_specs'])}")
