...
POOL_CSV = OUT_DIR / 'candidate_pool_v11.csv'
...
STAGE4_RESULT_FILES = [
    ...
    ROOT / 'data' / 'comsol_batch' / 'stage4_validation_ab_v10' / 'stage4_validation_results.csv',
]

# Keep the remaining-family line conservative by expanding only within a known rf09 neighborhood.
POINT_SPECS = [
    {
...
def build_candidate_pool(
    ...
    for seed in seed_manifest:
        ...
        for point in point_specs:
            ...
            rows.append({
                'sample_id': f"candidate_pool_v11_{point['candidate_point_id']}_{shape_id}",
    ...
    info = {
        ...
        'strategy': 'one best stage1-positive seed per never-stage4-validated family through v10, focused on the rf09 center point with the old rf09 anchor retained as a lightweight control arm',
    }
    return rows, info
...
def main() -> None:
    ...
    pool_rows, info = build_candidate_pool(shape_df, seed_manifest, POINT_SPECS, len(excluded_families))
    info['excluded_families'] = sorted(excluded_families)

    write_csv(POINT_MANIFEST, POINT_SPECS, [
    ...
    print(f'[DONE] point specs: {len(POINT_SPECS)}')
