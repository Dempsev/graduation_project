...
DEFAULT_SCORED_CSV = DEFAULT_OUT_ROOT / 'candidate_pool_seed_discovery_v11' / 'seed_discovery_predictions.csv'
DEFAULT_OUT_DIR = DEFAULT_OUT_ROOT / 'candidate_pool_seed_discovery_v11' / 'validation_manifest_v11'
...
def main() -> None:
    ...
    label = f'seed_only_refined_v11_primary_{int((selected["selection_bucket"] == "primary").sum())}_probe_{int((selected["selection_bucket"] == "probe").sum())}_diversity_{int((selected["selection_bucket"] == "diversity").sum())}'
    for idx, (_, row) in enumerate(selected.iterrows(), start=1):
        item = row.to_dict()
        item['validation_id'] = f'val{idx:03d}'
        item['selection_source'] = 'seed_only_refined_v11'
    ...
    manifest_csv = args.out_dir / 'comsol_validation_manifest_v11.csv'
    ordered_csv = args.out_dir / 'seed_only_refined_candidates_for_validation_v11.csv'
