...
DEFAULT_SCORED_CSV = DEFAULT_OUT_ROOT / 'candidate_pool_cascade_v3' / 'cascade_predictions.csv'
DEFAULT_OUT_DIR = DEFAULT_OUT_ROOT / 'candidate_pool_cascade_v3' / 'validation_manifest_v3'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Build v3 COMSOL validation manifest with family diversity limit.')
...
def select_ranked(df: pd.DataFrame, source_name: str, score_cols: List[str], top_k: int, max_per_family: int, gated_only: bool) -> List[Dict[str, object]]:
    ranked = df.copy()
    if gated_only:
...
def main() -> None:
    ...
    cascade_rows = select_ranked(df, 'cascade', ['cascade_gate', 'cascade_score', 'surrogate_pred_gap34_gain_Hz'], args.cascade_top_k, args.max_per_family, True)
    surrogate_rows = select_ranked(df, 'surrogate_only', ['surrogate_pred_gap34_gain_Hz', 'class_score'], args.surrogate_top_k, args.max_per_family, False)
    ...
    manifest_csv = args.out_dir / 'comsol_validation_manifest_v3.csv'
    ...
    write_csv(cascade_csv, cascade_rows, ['selection_source', 'selection_label', 'rank_within_source', *extra_fields, 'rank_cascade', 'rank_surrogate'])
    write_csv(surrogate_csv, surrogate_rows, ['selection_source', 'selection_label', 'rank_within_source', *extra_fields, 'rank_cascade', 'rank_surrogate'])
    ...
    print('[DONE] validation manifest v3 built')
