...
DEFAULT_SCORED_CSV = DEFAULT_OUT_ROOT / 'candidate_pool_cascade_v7' / 'cascade_predictions.csv'
DEFAULT_OUT_DIR = DEFAULT_OUT_ROOT / 'candidate_pool_cascade_v7' / 'validation_manifest_v7'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Build v7 COMSOL validation manifest for optional-family directional transfer probes.')
...
def main() -> None:
    ...
    for col in ['selection_priority', 'cascade_score', 'class_score', 'surrogate_pred_gap34_gain_Hz', 'contact_prob', 'positive_prob']:
    ...
    label = 'directional_transfer_probe_all' if args.top_k <= 0 else f'directional_transfer_probe_top_{args.top_k}'
    for idx, (_, row) in enumerate(sorted_df.iterrows(), start=1):
        item = row.to_dict()
        item['validation_id'] = f'val{idx:03d}'
        item['selection_source'] = 'directional_transfer_probe'
    ...
    manifest_csv = args.out_dir / 'comsol_validation_manifest_v7.csv'
    ordered_csv = args.out_dir / 'directional_transfer_candidates_for_validation.csv'
    ...
    summary = {
        ...
        'pm3_rows': int((pd.to_numeric(sorted_df['step_distance'], errors='coerce') == 3).sum()) if 'step_distance' in sorted_df.columns else None,
        'pm6_rows': int((pd.to_numeric(sorted_df['step_distance'], errors='coerce') == 6).sum()) if 'step_distance' in sorted_df.columns else None,
    }
    summary_json.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding='utf-8')

    print('[DONE] validation manifest v7 built')
