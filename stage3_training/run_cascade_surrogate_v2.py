...
DEFAULT_DATASET = ROOT / 'data' / 'ml_dataset' / 'v1' / 'candidate_pool_v1' / 'candidate_pool_v1.csv'
DEFAULT_CONTACT_RUN = DEFAULT_OUT_ROOT / 'mlp_contact_valid_parametric_core_v2_full'
DEFAULT_POSITIVE_RUN = DEFAULT_OUT_ROOT / 'mlp_is_positive_shape_parametric_core_v2_full'
DEFAULT_REG_RUN = DEFAULT_OUT_ROOT / 'mlp_gap34_gain_surrogate_v2_dataset_v2_full'


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run parameter-aware cascaded screening + surrogate scoring pipeline.')
    ...
    parser.add_argument('--run-name', default='candidate_pool_cascade_v2')
    parser.add_argument('--contact-threshold', type=float, default=0.58)
    parser.add_argument('--positive-threshold', type=float, default=0.70)
...
def main() -> None:
    ...
    print('[DONE] cascade v2 scoring complete')
    print(f'[RUN] {run_dir}')
