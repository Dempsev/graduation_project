...
DEFAULT_DATASET = ROOT / 'data' / 'ml_dataset' / 'v3' / 'candidate_pool_v3' / 'candidate_pool_v3.csv'
DEFAULT_CONTACT_RUN = DEFAULT_OUT_ROOT / 'mlp_contact_valid_parametric_core_v3_full'
DEFAULT_POSITIVE_RUN = DEFAULT_OUT_ROOT / 'mlp_is_positive_shape_parametric_core_v3_full'
DEFAULT_REG_RUN = DEFAULT_OUT_ROOT / 'mlp_gap34_gain_surrogate_v3_full'


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run classification-led cascade v3 on the candidate pool.')
    ...
    parser.add_argument('--run-name', default='candidate_pool_cascade_v3')
    parser.add_argument('--contact-threshold', type=float, default=0.55)
...
def build_point_summary(df: pd.DataFrame) -> List[Dict[str, object]]:
    rows = []
    for point_id, subset in df.groupby('point_id'):
        rows.append({
            'point_id': point_id,
            'main_id': subset['main_id'].iloc[0] if 'main_id' in subset.columns else '',
            'rows': int(len(subset)),
            'contact_gate_rate': float(subset['contact_gate'].mean()),
            'positive_gate_rate': float(subset['positive_gate'].mean()),
            'cascade_gate_rate': float(subset['cascade_gate'].mean()),
    ...
    rows.sort(key=lambda item: item['mean_cascade_score'], reverse=True)
    return rows
...
def build_shape_summary(df: pd.DataFrame) -> List[Dict[str, object]]:
    ...
    for shape_id, subset in df.groupby('shape_id'):
        rows.append({
            ...
            'rows': int(len(subset)),
            'max_contact_prob': float(np.max(subset['contact_prob'])),
            'max_positive_prob': float(np.max(subset['positive_prob'])),
            'max_class_score': float(np.max(subset['class_score'])),
            'mean_class_score': float(np.mean(subset['class_score'])),
            'mean_surrogate_pred_gap34_gain': float(np.mean(subset['surrogate_pred_gap34_gain_Hz'])),
...
def main() -> None:
    ...
    point_rows = build_point_summary(df)
    ...
    config = {
        ...
        'top_k': args.top_k,
        'score_definition': 'classification-led: contact_prob * positive_prob; regressor used as tie-break only',
    }
    ...
    print('[DONE] cascade v3 scoring complete')
    print(f'[RUN] {run_dir}')
