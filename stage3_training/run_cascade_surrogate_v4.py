...
DEFAULT_DATASET = ROOT / 'data' / 'ml_dataset' / 'v4' / 'candidate_pool_v4' / 'candidate_pool_v4.csv'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run split-arm cascade v4 on the candidate pool.')
    ...
    parser.add_argument('--run-name', default='candidate_pool_cascade_v4')
    parser.add_argument('--broad-contact-threshold', type=float, default=0.48)
    parser.add_argument('--broad-positive-threshold', type=float, default=0.70)
    parser.add_argument('--exploit-contact-threshold', type=float, default=0.50)
    parser.add_argument('--exploit-positive-threshold', type=float, default=0.95)
...
def ranked_frame(df: pd.DataFrame) -> pd.DataFrame:
    return df.sort_values(['cascade_gate', 'cascade_score', 'surrogate_pred_gap34_gain_Hz'], ascending=[False, False, False]).copy()


def assign_arm_thresholds(df: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    df = df.copy()
    broad_mask = df['pool_arm'].astype(str) == 'broad'
    df['contact_threshold'] = np.where(broad_mask, args.broad_contact_threshold, args.exploit_contact_threshold)
    df['positive_threshold'] = np.where(broad_mask, args.broad_positive_threshold, args.exploit_positive_threshold)
...
def main() -> None:
    ...
    df = assign_arm_thresholds(df, args)

    run_dir = DEFAULT_OUT_ROOT / args.run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    stage_rows = build_group_summary(df, 'source_stage', [])
    arm_rows = build_group_summary(df, 'pool_arm', ['source_role'])
    point_rows = build_group_summary(df, 'point_id', ['main_id', 'pool_arm', 'point_strategy'])
    shape_rows = build_group_summary(df, 'shape_id', ['shape_family', 'shape_role', 'pool_arm'])
    ...
    config = {
        ...
        'broad_contact_threshold': args.broad_contact_threshold,
        'broad_positive_threshold': args.broad_positive_threshold,
        'exploit_contact_threshold': args.exploit_contact_threshold,
        'exploit_positive_threshold': args.exploit_positive_threshold,
        'reg_min': args.reg_min,
        'top_k': args.top_k,
        'score_definition': 'split-arm classification-led: contact_prob * positive_prob; regressor used as tie-break only',
    }
    ...
    save_csv_rows(run_dir / 'cascade_arm_summary.csv', list(arm_rows[0].keys()) if arm_rows else ['pool_arm'], arm_rows)
    save_csv_rows(run_dir / 'cascade_point_summary.csv', list(point_rows[0].keys()) if point_rows else ['point_id'], point_rows)
    ...
    print('[DONE] cascade v4 scoring complete')
    print(f'[RUN] {run_dir}')
