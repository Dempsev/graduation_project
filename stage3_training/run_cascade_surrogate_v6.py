...
DEFAULT_DATASET = ROOT / 'data' / 'ml_dataset' / 'v6' / 'candidate_pool_v6_directional_targeted' / 'candidate_pool_v6.csv'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run v6 directional targeted cascade scoring.')
    ...
    parser.add_argument('--run-name', default='candidate_pool_cascade_v6')
...
def assign_targeted_thresholds(df: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    df['v5_reference_gain_Hz'] = pd.to_numeric(df['v5_reference_gain_Hz'], errors='coerce').fillna(-1.0)
...
def build_group_summary(df: pd.DataFrame, group_col: str, extra_cols: List[str]) -> List[Dict[str, object]]:
    ...
    for key, subset in df.groupby(group_col):
        row = {
            ...
            'mean_v5_reference_gain_Hz': float(np.mean(subset['v5_reference_gain_Hz'])),
        }
        for extra in extra_cols:
            if extra in subset.columns:
                row[extra] = subset[extra].iloc[0]
        rows.append(row)
    rows.sort(key=lambda item: (item.get('mean_v5_reference_gain_Hz', 0.0), item.get('mean_cascade_score', 0.0)), reverse=True)
    return rows
...
def main() -> None:
    ...
    seed_rows = build_group_summary(df, 'seed_shape_id', ['seed_family', 'preferred_direction'])
    rule_rows = build_group_summary(df, 'target_rule', ['preferred_direction'])
    ...
    config = {
        ...
        'score_definition': 'directional targeted: v5 physical reference priority first, model scores as confidence annotation',
    }
    ...
    print('[DONE] cascade v6 scoring complete')
    print(f'[RUN] {run_dir}')
