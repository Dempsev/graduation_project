...
DEFAULT_DATASET = ROOT / 'data' / 'ml_dataset' / 'v7' / 'candidate_pool_v7_directional_transfer_probe' / 'candidate_pool_v7.csv'
DEFAULT_CONTACT_RUN = DEFAULT_OUT_ROOT / 'mlp_contact_valid_parametric_directional_v5_full'
DEFAULT_POSITIVE_RUN = DEFAULT_OUT_ROOT / 'mlp_is_positive_shape_parametric_directional_v5_full'
DEFAULT_REG_RUN = DEFAULT_OUT_ROOT / 'mlp_gap34_gain_surrogate_v5_full'


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run v7 directional transfer probe cascade scoring.')
    ...
    parser.add_argument('--run-name', default='candidate_pool_cascade_v7')
...
def ranked_frame(df: pd.DataFrame) -> pd.DataFrame:
    return df.sort_values(
        ['selection_priority', 'cascade_gate', 'contact_prob', 'surrogate_pred_gap34_gain_Hz', 'positive_prob', 'class_score'],
...
def assign_thresholds(df: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    df['selection_priority'] = pd.to_numeric(df['selection_priority'], errors='coerce').fillna(99).astype(int)
...
def compute_gate_metrics(df: pd.DataFrame, top_k: int) -> Dict[str, object]:
    ...
    metrics: Dict[str, object] = {
        ...
        'top_k_seed_rows': int((ranked['is_seed_shape'].astype(str).isin(['1', '1.0', 'True', 'true'])).sum()),
        'top_k_pm3_rows': int((ranked['step_distance'] == 3).sum()),
        'top_k_pm6_rows': int((ranked['step_distance'] == 6).sum()),
    }
    return metrics
...
def build_group_summary(df: pd.DataFrame, group_col: str, extra_cols: List[str]) -> List[Dict[str, object]]:
    ...
    for key, subset in df.groupby(group_col):
        ...
        rows.append(row)
    rows.sort(key=lambda item: (item.get('mean_cascade_score', 0.0), item.get('mean_surrogate_pred_gap34_gain', 0.0)), reverse=True)
    return rows
...
def main() -> None:
    ...
    seed_rows = build_group_summary(df, 'seed_shape_id', ['seed_family', 'seed_tier'])
    rule_rows = build_group_summary(df, 'target_rule', [])
    ...
    config = {
        ...
        'top_k': args.top_k,
        'base_model_version': 'v5_directional',
        'score_definition': 'optional-family transfer probe; selection priority by seed/pm3/pm6, model scores used for ordering within each tier',
    }
    ...
    save_csv_rows(run_dir / 'cascade_rule_summary.csv', list(rule_rows[0].keys()) if rule_rows else ['target_rule'], rule_rows)
    ...
    print('[DONE] cascade v7 scoring complete')
    print(f'[RUN] {run_dir}')
