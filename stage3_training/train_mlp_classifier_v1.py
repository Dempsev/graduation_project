...
DEFAULT_DATASETS = {
    'contact_valid': ROOT / 'data' / 'ml_dataset' / 'v1' / 'tasks' / 'shape_screening_contact_cls_v1.csv',
    'is_positive_shape': ROOT / 'data' / 'ml_dataset' / 'v1' / 'tasks' / 'shape_screening_positive_cls_v1.csv',
}
...
ALLOWED_GROUP_KEYS = ['shape_family', 'none']


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train MLP classifiers for v1 shape-screening tasks.')
    parser.add_argument('--task', required=True, choices=['contact_valid', 'is_positive_shape'])
    parser.add_argument('--dataset', type=Path, default=None)
    parser.add_argument('--feature-preset', default='shape_only', choices=sorted(FEATURE_PRESETS.keys()))
    parser.add_argument('--group-keys', default='shape_family,none')
...
def resolve_run_name(args: argparse.Namespace) -> str:
    ...
    return f'mlp_{args.task}_shape_only_v1'
...
def train_model(model: nn.Module, train_loader, x_val: np.ndarray, y_val: np.ndarray, args: argparse.Namespace) -> tuple[nn.Module, List[Dict[str, float]]]:
    ...
    pos_count = float(np.sum(y_val == 1))
    # Use train distribution for imbalance handling when possible.
...
def save_predictions(path: Path, splits: Dict[str, pd.DataFrame], y_true: Dict[str, np.ndarray], y_prob: Dict[str, np.ndarray], task: str, threshold: float) -> None:
    ...
    save_csv_rows(path, ['split', 'sample_id', 'source_stage', 'shape_id', 'shape_family', 'target_name', 'y_true', 'y_prob', 'y_pred'], rows)
