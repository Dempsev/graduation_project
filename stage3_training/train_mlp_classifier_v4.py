...
DEFAULT_DATASETS = {
    'contact_valid': ROOT / 'data' / 'ml_dataset' / 'v4' / 'tasks' / 'parametric_contact_cls_v4.csv',
    'is_positive_shape': ROOT / 'data' / 'ml_dataset' / 'v4' / 'tasks' / 'parametric_positive_cls_v4.csv',
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train parameter-aware MLP classifiers for v4 tasks.')
    parser.add_argument('--task', required=True, choices=['contact_valid', 'is_positive_shape'])
    parser.add_argument('--dataset', type=Path, default=None)
    parser.add_argument('--feature-preset', default='parametric_core', choices=sorted(FEATURE_PRESETS.keys()))
...
def resolve_run_name(args: argparse.Namespace) -> str:
    ...
    return f'mlp_{args.task}_{args.feature_preset}_v4_full'
