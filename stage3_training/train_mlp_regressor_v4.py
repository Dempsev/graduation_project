...
DEFAULT_DATASET = ROOT / 'data' / 'ml_dataset' / 'v4' / 'tasks' / 'surrogate_regression_core_v4.csv'
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train cleaned surrogate MLP regressor for v4 datasets.')
    parser.add_argument('--dataset', type=Path, default=DEFAULT_DATASET)
    parser.add_argument('--feature-preset', default='surrogate_core', choices=sorted(FEATURE_PRESETS.keys()))
    ...
    parser.add_argument('--run-name', default='mlp_gap34_gain_surrogate_v4_full')
...
def main() -> None:
    ...
    if not feature_cols:
        raise RuntimeError('No usable feature columns found for regressor v4.')
