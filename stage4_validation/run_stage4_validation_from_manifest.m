function run_stage4_validation_from_manifest(cfg, startIndex, maxCount)
%RUN_STAGE4_VALIDATION_FROM_MANIFEST Shared mainline stage4 validation runner.
...
if ~isfile(cfg.validationManifestCsv)
    error('run_stage4_validation_from_manifest:MissingManifest', 'Validation manifest not found: %s', cfg.validationManifestCsv);
end
...
for i = 1:height(activeManifest)
    ...
    fprintf(['[%d/%d] %s source=%s shape=%s seed=%s offset=%+d point=%s ' ...
             'a1=%.3f a2=%.3f b2=%.3f a4=%.3f b5=%.3f\n'], ...
        i, height(activeManifest), char(string(row.validation_id(1))), char(string(row.selection_source(1))), ...
        char(string(row.shape_id(1))), char(string_or_empty(row, 'seed_shape_id')), ...
        numeric_or_zero(row, 'step_offset'), char(string(row.point_id(1))), ...
...
textVars = {'validation_id','selection_source','selection_label','sample_id','shape_id','shape_family','shape_role','candidate_id','main_id','point_id','pool_arm','point_strategy','family_prior_source','seed_prior_source','seed_shape_id','seed_family','seed_tier','seed_source','step_window','target_rule','preferred_direction','allowed_offsets','v5_reference_validation_id','stage1_reference_sample_id','stage1_reference_fourier_id','stage1_reference_candidate_tier'};
...
pointTable = manifestTable(mask, {'main_id','point_id','a1','a2','b1','b2','r0','a3','b3','a4','b4','a5','b5'});
end
...
pointSpec = struct( ...
    ...
    'a1', double(row.a1(1)), 'a2', double(row.a2(1)), 'b1', double(row.b1(1)), 'b2', double(row.b2(1)), 'r0', double(row.r0(1)), ...
...
if isempty(idx)
    error('run_stage4_validation_from_manifest:MissingBaselinePoint', 'No baseline reference found for point %s', pointId);
end
...
result.seed_shape_id = string_or_empty(row, 'seed_shape_id');
result.seed_family = string_or_empty(row, 'seed_family');
result.seed_step = numeric_or_nan_from_field(row, 'seed_step');
result.seed_tier = string_or_empty(row, 'seed_tier');
result.seed_source = string_or_empty(row, 'seed_source');
result.step_num = numeric_or_nan_from_field(row, 'step_num');
result.step_offset = numeric_or_nan_from_field(row, 'step_offset');
result.step_distance = numeric_or_nan_from_field(row, 'step_distance');
result.step_window = string_or_empty(row, 'step_window');
result.target_rule = string_or_empty(row, 'target_rule');
result.preferred_direction = string_or_empty(row, 'preferred_direction');
result.v5_reference_validation_id = string_or_empty(row, 'v5_reference_validation_id');
result.is_seed_shape = to_logical_or_false(row, 'is_seed_shape');
result.v5_reference_gain_Hz = numeric_or_nan_from_field(row, 'v5_reference_gain_Hz');
result.stage1_reference_sample_id = string_or_empty(row, 'stage1_reference_sample_id');
result.stage1_reference_fourier_id = string_or_empty(row, 'stage1_reference_fourier_id');
result.stage1_reference_gap_Hz = numeric_or_nan_from_field(row, 'stage1_reference_gap_Hz');
result.stage1_reference_gap_gain_Hz = numeric_or_nan_from_field(row, 'stage1_reference_gap_gain_Hz');
result.stage1_reference_contact_length = numeric_or_nan_from_field(row, 'stage1_reference_contact_length');
result.stage1_reference_candidate_tier = string_or_empty(row, 'stage1_reference_candidate_tier');
result.pool_arm = string_or_empty(row, 'pool_arm');
result.point_strategy = string_or_empty(row, 'point_strategy');
...
function value = to_logical_or_false(row, fieldName)
...
function value = numeric_or_nan_from_field(row, fieldName)
if ismember(fieldName, row.Properties.VariableNames)
...
function value = numeric_or_zero(row, fieldName)
value = numeric_or_nan_from_field(row, fieldName);
if isnan(value)
    value = 0;
end
end

function value = string_or_empty(row, fieldName)
if ismember(fieldName, row.Properties.VariableNames)
...
stringFields = { ...
    ...
    'seed_shape_id','seed_family','seed_tier','seed_source','step_window','target_rule','preferred_direction','v5_reference_validation_id', ...
    'stage1_reference_sample_id','stage1_reference_fourier_id','stage1_reference_candidate_tier', ...
    'shape_id','shape_family','shape_role','candidate_id','main_id','point_id','pool_arm','point_strategy','family_prior_source','seed_prior_source','material_case','error_message' ...
};
logicalFields = {'is_seed_shape','contact_gate','positive_gate','reg_positive_gate','cascade_gate','geometry_valid','contact_valid','has_tiny_fragments','solve_success'};
out = struct();
